...
public class CartPage extends BasePage {
    ...
    /**
     * Removes all items from the cart by clicking each Delete link.
     * DemoBlaze persists cart data server-side, so old items can pile up
     * across test runs if we don't clean up.
     */
    public void clearCart() {
        waitForCartToLoad();
        List<WebElement> deletes = driver.findElements(deleteLinks);
        while (!deletes.isEmpty()) {
            deletes.get(0).click();
            WaitHelper.briefPause(1500);
            deletes = driver.findElements(deleteLinks);
        }
    }
